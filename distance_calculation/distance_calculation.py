...
class GMT456Plugin:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'GMT456Plugin_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Distance Calculation')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('GMT456Plugin', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/distance_calculation/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Find shortest distance and actual distance'),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginVectorMenu(
                self.tr(u'&Distance Calculation'),
                action)
    ...
    def shapefile_sec(self):
        try:
            self.shapefilePath, shapefileType = QFileDialog.getOpenFileName(self.dlg, "Select the shapefile", "", "ESRI Shapefiles(*.shp *.SHP);; GeoJSON (*.GEOJSON *.geojson);; Geography Markup Language(*.GML)")
